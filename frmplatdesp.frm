...
Begin VB.Form frmplatdesp 
   BackColor       =   &H00FFFFFF&
   Caption         =   "Platos despachados"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   4680
...
pub_cadena = "SELECT * FROM PEDIDOS WHERE PED_CODCIA = ? and PED_fecha >= ? and PED_fecha <= ?  AND PED_CANATEN <> 0 "
...
pub_cadena = "SELECT DISTINCT PED_CODART  FROM PEDIDOS WHERE PED_CODCIA = ? and PED_fecha >= ? and PED_fecha <= ? AND PED_CANATEN  <> 0 "
...
pub_cadena = "SELECT PED_CANTIDAD,PED_CODCLIE,PED_HORA,PED_CODVEN FROM PEDIDOS WHERE PED_CODCIA = ? AND PED_FECHA >= ? AND PED_FECHA <= ? AND  PED_CODART = ? AND  PED_ESTADO = 'N'  AND PED_CANATEN  <> 0 ORDER BY PED_HORA ASC"
...
pub_cadena = "SELECT sum(PED_CANTIDAD) as total  FROM PEDIDOS WHERE PED_CODCIA = ? AND PED_FECHA >= ? AND PED_FECHA <= ?  AND  PED_CODART = ? AND PED_ESTADO = 'N' AND PED_CANATEN <> 0"
